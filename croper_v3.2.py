...
def handle_keypress(key):
    if key == ord('r'):  # 'r' for rotate
        rotate_image()
    elif key == ord('c'):  # 'c' for crop
        crop_image()
    elif key == ord('z'):  # 'z' for redo
        redo_image()
    elif key == ord('s'):  # 's' for save
        save_file(warped if warped is not None else image)
...
def main():
    ...
    rotate_button = Button(button_frame, text="Rotate (r)", command=rotate_image, width=15, height=2)
    rotate_button.pack(side="bottom", padx=5)

    crop_button = Button(button_frame, text="Crop (c)", command=crop_image, width=15, height=2)
    crop_button.pack(side="bottom", padx=5)
    
    redo_button = Button(button_frame, text="Redo (z)", command=redo_image, width=15, height=2)
    redo_button.pack(side="bottom", padx=5)
    
    save_button = Button(button_frame, text="Save (s)", command=lambda: save_file(warped if warped is not None else image), width=15, height=2)
    ...
    # Start Tkinter mainloop without blocking using root.update()
    while True:
        # Check for OpenCV keyboard events
        key = cv2.waitKey(1) & 0xFF
        if key == 27:  # Escape key to exit
            break
        elif key != 255:  # Handle key presses
            handle_keypress(key)
